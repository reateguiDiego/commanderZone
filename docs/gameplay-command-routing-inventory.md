...
|---|---|---:|---:|---:|---:|---:|---|---|
| Runtime WebSocket final hot path | Go `game-runtime` `/ws` `command.v2` | YES | NO | NO | NO | NO | Runtime ticket from Symfony, `GAME_RUNTIME_ENABLED`, `GAMEPLAY_V2_COMMANDS_ALLOWLIST`, frontend gameplay v2, `GAME_RUNTIME_OWNERSHIP_MODE=single-node|postgres-lease` | Normal migrated gameplay commands execute in the runtime actor and emit `patch.v2`. Reconnect replay uses `lastAppliedVersion`; resync is exceptional. Signed ticket `playerId` is the actor id; player-scoped and hidden-zone commands are rejected when they target another player. Commands are rejected with `OWNERSHIP_NOT_HELD` when the local runtime process cannot prove ownership. In `postgres-lease`, event append validates `fencing_token` against `game_runtime_lease`. |
| Runtime HTTP internal command path | Go `game-runtime` `/commands` | YES | NO | NO | NO | NO | Internal Symfony client, `GAME_RUNTIME_ENABLED`; `GAME_RUNTIME_ALLOW_INITIAL_STATE_COMMANDS` only enables rejected legacy migration payloads for controlled tests/migration; `GAME_RUNTIME_OWNERSHIP_MODE=single-node|postgres-lease` | Direct runtime command endpoint for Symfony/runtime tests and internal dispatch. It rejects `initialState` unless the explicit env flag is enabled. Actor-level checks reject mismatched `actorId` for player-scoped commands. Commands are rejected with `ownership_not_held` when the local runtime process cannot prove ownership. In `postgres-lease`, event append validates `fencing_token` against `game_runtime_lease`. |
...
| Concede lifecycle runtime path | `game.concede` | YES when allowlisted | Legacy only outside runtime-primary or emergency PHP path | Runtime-final: NO; PHP compatible path: YES | Runtime-final: NO; PHP compatible path: YES | Runtime-final: NO; PHP compatible path: explicit emergency only | `GAME_RUNTIME_ENABLED`, `GAMEPLAY_V2_COMMANDS_ALLOWLIST` | `game.concede` requires the signed `command` permission and runtime actor validation for self-concede. When one `active` player remains, Go persists the final concede event and hands off `game.finished` to Symfony. |
...
- Runtime `/ws` and `/commands` must fail closed with explicit ownership errors when the local runtime process cannot prove ownership for `gameId`.
