...
The generator reads `SEO_ROUTES` and `SUPPORTED_LOCALES`, writes `sitemap-index.xml` and `sitemaps/sitemap-seo.xml`, and includes every localized SEO URL with self-referencing `hreflang`, all locale alternates, and `x-default`. Community routes, public decks, top lists, cards, commanders and users are deliberately excluded until their sitemap is generated by the backend.
