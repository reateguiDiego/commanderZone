...
describe('legal routes', () => {
  it('defines localized legal paths for every legal page and SEO locale', () => {
    ...
    expect(getLegalPath('contact', 'en')).toBe('/contact/');
    ...
    expect(getLegalPath('contact', 'pt')).toBe('/pt/contato/');
    expect(getLegalPath('contact', 'it')).toBe('/it/contatto/');
  });
  ...
  it('keeps legal prerender routes separate from SEO indexable routes', () => {
    ...
    expect(LEGAL_PRERENDER_ROUTES).toContain('/es/contacto/');
  ...
  it('centralizes the legal contact email', () => {
    expect(LEGAL_CONTACT_EMAIL).toBe('support@commanderzone.com');
  });
});
