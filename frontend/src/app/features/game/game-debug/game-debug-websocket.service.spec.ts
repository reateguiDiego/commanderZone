...
describe('GameDebugWebsocketService', () => {
  ...
  it('keeps websocket base path prefix when building the debug endpoint URL', async () => {
    gamesApi.websocketTicket.mockReturnValue(of({
      ticket: 'ticket-1',
      expiresAt: '2026-01-01T00:00:30+00:00',
      websocketUrl: 'wss://api.commanderzone.com/ws-game/games/game-1?ticket=ticket-1',
    }));

    await service.connect('game-1', vi.fn(), vi.fn());

    expect(sockets).toHaveLength(1);
    expect(sockets[0].url).toBe('wss://api.commanderzone.com/ws-game/games/game-1/debug?ticket=ticket-1');
  });

  it('builds debug endpoint URL without prefix when ticket URL has no base path', async () => {
    gamesApi.websocketTicket.mockReturnValue(of({
      ticket: 'ticket-1',
      expiresAt: '2026-01-01T00:00:30+00:00',
      websocketUrl: 'wss://api.commanderzone.com/games/game-1?ticket=ticket-1&lastSeenVersion=22',
    }));

    await service.connect('game-1', vi.fn(), vi.fn());

    expect(sockets).toHaveLength(1);
    expect(sockets[0].url).toBe('wss://api.commanderzone.com/games/game-1/debug?ticket=ticket-1');
  });
});
