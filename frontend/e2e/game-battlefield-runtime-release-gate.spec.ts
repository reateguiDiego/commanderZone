...
test.describe('battlefield/counters runtime release gate', () => {
  ...
  test('real browser sessions apply battlefield/counters runtime patch.v2 without snapshot refetch', async ({
    browser,
    request,
    baseURL,
  }) => {
    ...
    if (battlefieldIds.length < 3) {
      throw new Error(
        `Expected at least 3 hand cards for battlefield gate setup, got ${battlefieldIds.length}.`,
      );
    }
...
${(
  await pageB
    ...
    .catch(() => '')
      ...
      await expect(battlefieldCard(pageB, playerA.user.id, battlefieldIds[0])).toBeVisible({
        timeout: 15_000,
      });
      ...
      await waitForPatchV2(framesB, (patch) =>
        hasFieldPatch(patch, battlefieldIds[0], { tapped: true }),
      );
      await expect
        .poll(
          async () =>
            (await battlefieldCard(pageA, playerA.user.id, battlefieldIds[0]).getAttribute(
              'class',
            )) ?? '',
        )
        .toContain('tapped');
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        ownerPatch: (patch) =>
          hasFieldPatch(patch, battlefieldIds[0], { position: { x: 0.4, y: 0.2, unit: 'ratio' } }),
      });
      nextBaseVersion = outcome.version;
      expect(snapshotRefetches).toBe(refetchBaseline);
      ...
      nextBaseVersion = outcome.version;
      expect(operation(outcome.patch, 'card.counters.patch')?.['counters']).toMatchObject({
        charge: 2,
      });
      expect(snapshotRefetches).toBe(refetchBaseline);
      ...
      nextBaseVersion = outcome.version;
      expect(operation(outcome.patch, 'player.counters.set')?.['counters']).toMatchObject({
        poison: 2,
      });
      expect(snapshotRefetches).toBe(refetchBaseline);
      ...
      nextBaseVersion = outcome.version;
      expect(operation(outcome.patch, 'turn.set')?.['turn']).toMatchObject({
