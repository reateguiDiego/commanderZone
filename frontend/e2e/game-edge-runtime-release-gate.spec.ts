...
test.describe('edge gameplay runtime release gate', () => {
  ...
  test('real browser sessions apply edge runtime patch.v2 without leaks or snapshot refetch', async ({ browser, request, baseURL }) => {
    ...
    if (handIds.length < 5) {
      throw new Error(`Expected at least 5 hand cards for edge runtime gate, got ${handIds.length}.`);
    }
...
${(await pageA.locator('body').innerText().catch(() => '')).slice(0, 2000)}`);
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        payload: { playerId: playerA.user.id, instanceId: battlefieldIds[0], targetPlayerId: playerA.user.id },
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        ownerPatch: (patch) => hasOp(patch, 'zone.random_card.selected') && JSON.stringify(patch).includes(randomHandId),
      });
      nextBaseVersion = outcome.version;
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        payload: { playerId: playerA.user.id, instanceId: battlefieldIds[0], position: { x: 0.2, y: 0.4, unit: 'ratio' } },
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        ownerPatch: (patch) => hasOp(patch, 'zone.cards.add') && JSON.stringify(patch).includes(putTopId),
      });
      nextBaseVersion = outcome.version;
      assertNoStaticPayload(outcome.patch);
      expect(snapshotRefetches).toBe(refetchBaseline);
      ...
      outcome = await sendRuntimeCommandAndWait(commandPage, ticket.websocketUrl, framesA, {
        ...
        ownerPatch: (patch) => hasOp(patch, 'zone.cards.add') && JSON.stringify(patch).includes(putBottomId),
      });
      nextBaseVersion = outcome.version;
      assertNoStaticPayload(outcome.patch);
      expect(snapshotRefetches).toBe(refetchBaseline);
...
async function sendHttpCommand(request: APIRequestContext, gameId: string, token: string, type: string, payload: JsonObject): Promise<void> {
