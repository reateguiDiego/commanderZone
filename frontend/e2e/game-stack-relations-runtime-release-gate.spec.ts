...
test.describe('stack/relations runtime release gate', () => {
  ...
  test('real browser sessions apply stack/relation/helper patch.v2 without snapshot refetch', async ({
    browser,
    request,
    baseURL,
  }) => {
    ...
    if (battlefieldIds.length < 3) {
      throw new Error(
        `Expected at least 3 hand cards for stack/relations gate setup, got ${battlefieldIds.length}.`,
      );
    }
...
${(
  await pageB
    ...
    .catch(() => '')
      ...
      const stackId =
        stringFromNestedOperation(outcome.patch, 'stack.item.add', ['item', 'stackId']) ??
      ...
      assertNoStaticPayload(outcome.patch);
      const attachmentId = stringFromNestedOperation(outcome.patch, 'attachment.add', [
        'attachment',
        'id',
      ]);
      ...
      await expect(
        pageA
          .getByTestId('special-entity-strip')
          .filter({ hasText: /City|Blessing/i })
          .first(),
...
function stringFromNestedOperation(
  message: JsonObject,
  op: string,
