...
final class DeckPowerAnalyzer
{
     ...
     * @return array{band:string,confidence:string,signals:array<string,int>,evidence:list<string>,notes:list<string>}
     */
    public function analyze(array $metrics, array $resolvedCards, array $combos): array
    {
        $signals = $this->signals($metrics, $resolvedCards, $combos);
        $score = 0;
        $evidence = [];
     ...
     */
    private function signals(array $metrics, array $resolvedCards, array $combos): array
    {
        $signals = [
            'fastMana' => $metrics['roles']['fastMana'] ?? 0,
            'efficientTutors' => 0,
            'freeInteraction' => 0,
            'compactWincons' => 0,
            ...
            'highPowerStaples' => 0,
            'gameChanger' => 0,
            'strongTutors' => ($metrics['quality']['tutor']['premium'] ?? 0) + ($metrics['quality']['tutor']['good'] ?? 0),
            'strongProtection' => ($metrics['quality']['protection']['premium'] ?? 0) + ($metrics['quality']['protection']['good'] ?? 0),
        ];
        ...
        foreach ($resolvedCards as $card) {
            ...
            $signals['lowOpportunityCost'] += isset($flags['low_opportunity_cost']) ? $quantity : 0;
            $signals['cedhStaples'] += ($this->boolPath($profile, ['flags', 'cedhStaple']) || isset($flags['cedh_staple'])) ? $quantity : 0;
            $signals['manaPositiveComboPieces'] += isset($flags['mana_positive_combo_piece']) ? $quantity : 0;
            $signals['highPowerStaples'] += isset($flags['high_power_staple']) ? $quantity : 0;
            $signals['gameChanger'] += ($profile['isGameChanger'] ?? false) === true || isset($flags['manual_game_changer']) ? $quantity : 0;
        }
