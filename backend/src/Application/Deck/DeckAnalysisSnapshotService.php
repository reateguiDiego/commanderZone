...
{
    private const ANALYZER_VERSION = 'basic_deck_analysis_v1';
    ...
    public function analyze(Deck $deck, DeckAnalysisService $analysis, array $options = []): array
    {
        ...
        $result = $analysis->analyze($deck, $normalizedOptions);
    ...
    {
        $context = $this->context($deck, ['view' => 'bracket']);
        $existing = $this->snapshotRow($deck->id(), $context['options_hash']);
        $staleReason = $this->staleReason($existing, $context);
        ...
        $result = ['bracket' => $bracketSignalProvider->analysis($deck)['bracket']];
        $saved = $this->saveSnapshot($deck->id(), $context, $result, $existing);
        $result['snapshot'] = $this->metadata(false, $staleReason ?? 'missing', $saved, $context);
